...
function assertStaticAdminGuardrails() {
    ...
    assertIncludes(payloadScript, 'assertVisibleSwitchHitTargets(adminSettingsSource)', 'payload guardrail verifies visible control hit targets')
    assertIncludes(payloadScript, 'control-hit-target', 'payload guardrail keeps visible text/icon click wrappers')
    assertIncludes(payloadScript, '@click.capture.stop.prevent', 'payload guardrail keeps wrapper click handling')
    assertIncludes(payloadScript, 'must not rely on wrapperElement', 'payload guardrail rejects unsupported wrapperElement reliance')
    assertIncludes(payloadScript, "assert.deepEqual(VALID_INITIAL_PASSWORD_POLICIES, ['random'])", 'payload guardrail verifies active random-only initial password policy choices')
    ...
    assertIncludes(frontendInitialStateService, String.raw`[?&](?:api[_-]?key|token|password|secret|authorization|immich[_-]?username|immich[_-]?password|immichUsername|immichPassword)=`, 'Initial-state redaction must cover query-string credentials')
    assertIncludes(frontendInitialStateService, String.raw`"(?:password|admin_api_key|immich_username|immich_password|immichUsername|immichPassword|apiKey|api_key|x-api-key|token|secret|authorization)"\s*:\s*"`, 'Initial-state redaction must cover JSON credential strings')
    ...
    assertIncludes(frontendInitialStateService, String.raw`(api[_-]?key|token|password|secret|authorization|immich[_-]?username|immich[_-]?password|immichUsername|immichPassword)(\s*[=:]\s*)`, 'Initial-state redaction must cover generic key/value secrets')
    ...
    assertIncludes(adminSettingsControllerTest, 'testGetConfigNormalizesLegacyPasswordPolicyAndRedactsCredentialFields', 'Admin settings tests normalize initial_password_policy while redacting credentials')
    assertIncludes(adminSettingsControllerTest, "$this->assertSame('random', $config['initial_password_policy']);", 'Admin settings tests assert normalized initial_password_policy is not redacted by key name')
    ...
    const routesSource = readProjectFile('appinfo/routes.php')
    assertIncludes(routesSource, "['name' => 'auth_handoff#openImmich', 'url' => '/auth/open-immich', 'verb' => 'GET']", 'Auto-login handoff route must remain GET-only')
    const authHandoffController = readProjectFile('lib/Controller/AuthHandoffController.php')
    assertIncludes(authHandoffController, 'resolveLegacyPasswordLoginHandoff($this->userId)', 'Auto-login controller must resolve stored user login credentials server-side for the active user')
    assertIncludes(authHandoffController, 'sharedParentDomainCookies((string)$session[\'setCookie\'], $redirectUrl)', 'Open Immich controller must rewrite cookies only through the shared-parent-domain guard')
    assertIncludes(authHandoffController, 'sharedParentDomain($nextcloudHost, $immichHost)', 'Open Immich controller must validate the shared parent domain before setting Immich cookies')
    assertIncludes(authHandoffController, 'immich_auth_type=password;', 'Open Immich controller must set password auth type cookie')
    assertIncludes(authHandoffController, 'immich_is_authenticated=true;', 'Open Immich controller must set authenticated-state cookie')
    assertIncludes(authHandoffController, 'isSafeRedirectUrl($redirectUrl)', 'Open Immich controller must validate redirect targets')
    assert.ok(!authHandoffController.includes('admin_api_key'), 'Auto-login controller must not use admin API keys for web login')
    const sharedCookieResponse = readProjectFile('lib/Http/SharedCookieRedirectResponse.php')
    assertIncludes(sharedCookieResponse, "header('Set-Cookie: ' . $setCookie, false)", 'Shared cookie redirect response must append multiple Set-Cookie headers without replacement')
    const syncStateSource = readProjectFile('lib/Db/SyncState.php')
    assert.ok(!syncStateSource.includes("'immichUsername' =>"), 'SyncState JSON must not expose Immich usernames')
    assert.ok(!syncStateSource.includes("'immichPassword' =>"), 'SyncState JSON must not expose Immich passwords')
