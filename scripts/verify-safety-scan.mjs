...
function assertStaticAdminGuardrails() {
    ...
    assertIncludes(authHandoffController, 'resolveAutoLoginHandoff($this->userId)', 'Auto-login controller must resolve credentials server-side for the active user')
    assertIncludes(authHandoffController, 'createImmichLoginSession($handoff)', 'Auto-login controller must perform login server-side')
