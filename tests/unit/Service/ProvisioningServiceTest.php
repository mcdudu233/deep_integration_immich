...
class ProvisioningServiceTest extends TestCase {
    ...
    public function testRefusesStorageLabelChangeAfterAssetsExist(): void {
        ...
        $this->immichUserAdminService->method('getUserQuotaUsage')->with('immich-alice')->willReturn(10);
        $this->immichUserAdminService->expects($this->never())->method('updateUser');
        $this->syncStateService->expects($this->once())
            ->method('updateMapping')
            ->with('alice', $this->callback(function (array $fields): bool {
                $this->assertSame(SyncStateService::STATUS_FAILED, $fields['lastSyncStatus']);
                $this->assertSame('Refusing to change Immich storage label after assets exist.', $fields['lastError']);
                return true;
            }));
        ...
        $this->assertSame('skipped', $result['action']);
        $this->assertSame(['Refusing to change Immich storage label after assets exist.'], $result['errors']);
    }
