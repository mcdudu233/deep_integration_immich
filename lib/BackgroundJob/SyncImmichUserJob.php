...
class SyncImmichUserJob extends QueuedJob {
    ...
    protected function run($argument): void {
        try {
            ...
            $this->logger->warning('Immich user sync job failed before a user could be resolved: ' . $error, [
    ...
    public function syncForUser(string $ncUid): array {
        $ncUid = trim($ncUid);
        if ($ncUid === '') {
            return $this->failureResult('', 'SyncImmichUserJob requires a non-empty ncUid argument.');
        }
        ...
        try {
            ...
            $this->logger->warning('Immich user sync job failed for Nextcloud user "' . $ncUid . '": ' . $error, [
                'app' => Application::APP_ID,
                'ncUid' => $ncUid,
                'safeUpdatesOnly' => true,
            ]);
    ...
    private function extractNcUid(mixed $argument): string {
        ...
        $ncUid = trim($ncUid);
        if ($ncUid === '') {
            throw new \InvalidArgumentException('SyncImmichUserJob requires a non-empty ncUid argument.');
        }
    ...
    private function normaliseReconcileResult(string $ncUid, array $result): array {
        ...
        if ($errors !== []) {
            $this->logger->warning('Immich user sync job completed with errors for Nextcloud user "' . $ncUid . '": ' . implode('; ', $errors), [
                'app' => Application::APP_ID,
                'ncUid' => $ncUid,
                'safeUpdatesOnly' => true,
            ]);
